/**
 * `dash4devs status` — show the active deploy and recent history for this platform.
 */

import chalk from "chalk";
import { loadConfig } from "../config.js";
import { createApi } from "../api.js";
import { step, formatBytes, warn } from "../ui.js";
...
export async function run() {
  ...
  if (!res.active) {
    warn("No active deployment for this platform yet");
  } else {
  ...
  if (res.recent && res.recent.length > 1) {
    step("Recent");
      ...
      const badge =
        d.status === "active"
          ...
          : d.status === "superseded"
            ...
            : chalk.yellow("○");
      console.log(
        `     ${badge} ${d.deploy_id}  ${chalk.dim(d.status)}  ${chalk.dim(d.created_at)}`,
      );
    }
  }
}
